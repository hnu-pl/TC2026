...
#pagebreak()
정규 언어(regular language)란 어떤 (결정적) 유한 오토마타(DFA) $M$으로 인식할 수 있는 언어. \
...
+ $A$와 $B$가 정규언어이면 정규언어의 정의에 따라 \
  $A=L(M_1)$이고 $B=L(M_2)$인 DFA $M_1$과 $M_2$가 존재
+ $A union B = C$를 DFA 2개를 동시에 활용해서 인식하는 것은 다음과 같이 가능한데
   - $s in C$를 $M_1$이 인식하는지 한번 돌려보고
   - 같은 s를 $M_2$가 인식하는지 또 한 번 돌려봐서
   - 둘 중 하나만 인식에 성공하면 합격!
  그런데 문제는 이런 방식은 정규 언어의 정의와 정확히 맞지는 않음. \
  왜냐하면 정규언어란 정확히 1개의 DFA로 인식을 해야 되니까
+ 그러니까 방금 위에서 두 개의 기계로 하던 일을 똑같이 할 수 있는
  하나의 DFA $M$이 존재해서\
  이거 하나로 $C$를 인식하는, 
  즉 $L(M) = C$가 되는 $M$을 찾으면(만들어내면) 증명이 되는 것
